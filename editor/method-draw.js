		...
		Editor.init = function() {
			...
			(function() {
				...
				var dbox = function(type, msg, callback, defText, callback2) {
					...
					if(type == 'newSvgClean' || type == 'newSvgDirty') {
						//$('#dialog_container').css("width", "500px");
						$('#dialog_header').html('<h3 class="dialog__header">Create New Sketch Annotation</h3>');
						var buttonText = (type == 'newSvgClean') ? uiStrings.common.saveNew:uiStrings.common.saveNewDirty;
					} else {
						buttonText = uiStrings.common.ok
					}
					...
					if(type == 'newSvgDirty') {
						discard = $('<button/>', {
							text: uiStrings.common.discardNew,
							class: "button button--secondary"
						}).appendTo(btn_holder).on("click touchstart", function () {
							box.hide();
				...
				$.newSvgClean = function(msg, cb, cb2) {	dbox('newSvgClean', msg, cb);};
				$.newSvgDirty = function(msg, cb, cb2) {	dbox('newSvgDirty', msg, cb, '', cb2);};
...
// 						|| elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
        ...
        svgcanvas.addEventListener('touchmove', start_pan);
			...
			var pasteSelected = function() {
				flash($('#edit_menu'));
				svgCanvas.pasteElements('offset');
			}
			...
			var clickClearDirty = function(){
				var dims = curConfig.dimensions;
				$.newSvgDirty(uiStrings.notification.QwantToClear, function(ok) {
						if(!ok) return;
						if(inIframe()) {
							window.methodDraw.parent.save();
							window.methodDraw.parent.newSketch();
						} else {
							notInIframe();
						}
					},
			...
			var clickClean = function(){
				$.newSvgClean(uiStrings.notification.QwantToClear, function(ok) {
						if(!ok) return;
						if(inIframe()) {
							window.methodDraw.parent.newSketch();
						} else {
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
			...
			var Actions = function() {
				...
				var tool_buttons = [
					...
					{sel:'#tool_clear', fn:
						function() {
							if (svgCanvas.undoMgr.getUndoStackSize() > 0) { //if true - some changes were made on the drawing
								clickClearDirty();
							} else {
								clickClean();
							}
